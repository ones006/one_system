...
class One_auth_model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		parent::__construct();
		$this->load->database();
		$this->load->config('one_auth', TRUE);
		$this->load->helper('cookie');
		$this->load->helper('date');
		$this->load->library('session');
		$this->lang->load('one_auth');

		//initialize db tables data
		$this->tables  = $this->config->item('tables', 'one_auth');

		//initialize data
		$this->identity_column = $this->config->item('identity', 'one_auth');
		$this->store_salt      = $this->config->item('store_salt', 'one_auth');
		$this->salt_length     = $this->config->item('salt_length', 'one_auth');
		$this->join			   = $this->config->item('join', 'one_auth');

		//initialize messages and error
		$this->messages = array();
		$this->errors = array();
		$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'one_auth');
		$this->message_end_delimiter   = $this->config->item('message_end_delimiter', 'one_auth');
		$this->error_start_delimiter   = $this->config->item('error_start_delimiter', 'one_auth');
		$this->error_end_delimiter     = $this->config->item('error_end_delimiter', 'one_auth');
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$default_group = $this->where('name', $this->config->item('default_group', 'one_auth'))->group()->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($user->password === $password)
			{
                ...
                if ($remember && $this->config->item('remember_users', 'one_auth'))
                {
	...
	**/
	...
	public function set_lang($lang = 'en')
	{
		...
		set_cookie(array(
			'name'   => 'lang_code',
			'value'  => $lang,
			'expire' => $this->config->item('user_expire', 'one_auth') + time()
		));
	...
	private function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			set_cookie(array(
				...
				'expire' => $this->config->item('user_expire', 'one_auth'),
			));
	...
	public function login_remembered_user()
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->config->item('user_extend_on_login', 'one_auth'))
			{
				$this->remember_user($user->id);
			}
